...
[Route("api/v1/ai")]
[Tags("AI")]
public class ChatController(Kernel kernel, IChatCompletionService chatCompletionService, BodybuildingCoachPlugin bodybuildingPlugin)
    : ControllerBase
{
    /// <summary>
    ...
    public async Task<IActionResult> Ask([FromBody] AskRequest? request)
    {
        ...
        try
        {
                ...
                request.Measurement.Waist.HasValue)
            {
                ...
                toolOutputs.Add($"V-Taper: {vtaper} (chest: {mm.Chest} cm, waist: {mm.Waist} cm, ratio: {(mm.Waist > 0 ? (mm.Chest / mm.Waist).ToString("0.00") : "n/a")})");
            }
            ...
            if (request.RecentWeights is not null && request.RecentWeights.Count >= 2 && request.RepsVolume.HasValue)
            {
                var weights = request.RecentWeights
                    .Where(w => w != null && w.Weight.HasValue && w.Date.HasValue)
