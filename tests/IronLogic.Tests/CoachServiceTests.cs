...
public class CoachServiceTests
{
    ...
    public async Task GenerateAdviceAsync_WhenVolumeIsLow_ShouldContainVolumeWarning()
    {
        ...
        double vTaper = mockAnalysis.Object.CalculateChestToWaistRatio(new MuscleMeasurement { Chest = 100, Waist = 90 });
        double lowMonthlyVolume = 50_000; // below the 100k threshold in the service

        ICoachService coachService = new CoachService();

        // Act
        var advice = await coachService.GenerateAdviceAsync(vTaper, lowMonthlyVolume, userName: "UnitTest");

        // Assert
        advice.Should().Contain("Volume is on the lower side", because: "the monthly volume passed is below the low-volume threshold");
    }
}