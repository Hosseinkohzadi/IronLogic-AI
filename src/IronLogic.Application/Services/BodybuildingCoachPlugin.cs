...
public class BodybuildingCoachPlugin
{
    ...
    [KernelFunction]
    [Description(
        "Calculates chest-to-waist ratio; returns 'Pro Level' when ratio > 1.4 otherwise 'Improvement Needed'.")]
    public string AnalyzeVTaper(MuscleMeasurement measurement)
    {
        ...
        if (measurement.Waist <= 0f)
            throw new ArgumentException("Waist must be a positive value.", nameof(measurement));
    ...
    ///     Numeric training volume (for example total reps*weight or aggregated daily reps-volume) for the same observation
    ///     window.
    ...
    ///     Thrown when <paramref name="recentWeights" /> has fewer than 2 entries or when
    ///     <paramref name="repsVolume" /> is negative.
    /// </exception>
    /// <remarks>
    ...
    public string SmartCutAdvice(IEnumerable<DailyWeight> recentWeights, double repsVolume)
    {
        ...
        if (weights.Count < 2)
            throw new ArgumentException("At least two weight entries are required to evaluate a trend.",
                nameof(recentWeights));

        var first = weights.First().Weight;
        var last = weights.Last().Weight;

        if (first <= 0f)
            throw new ArgumentException("Historical weight values must be positive.", nameof(recentWeights));
        ...
        if (weightChangePercent > significantChange)
        {
            ...
            if (repsVolume >= highVolumeThreshold)
                return
                    "Maintain calories and monitor for 1-2 weeks — weight is up but training volume is very high (possible muscle/neuromuscular gains).";
        ...
        if (weightChangePercent < -significantChange)
            return
                "Do not drop calories — weight is decreasing. Ensure recovery and adequate protein; consider small increase if energy is low.";
