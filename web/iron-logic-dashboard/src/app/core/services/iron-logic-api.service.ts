...
export class IronLogicApiService {
  ...
  getWorkoutStatsWithAdvice(): Observable<WorkoutStats> {
    if (!this.statsCache$) {
      this.statsCache$ = this.http.get<WorkoutStats>(this.statsUrl).pipe(shareReplay(1));
    }
  ...
  getUserTransactions(userId: string): Observable<any[]> {
    ...
    // In production, this would query: iron-logic-api/transactions?userId={userId}
    ...
    // Filter transactions by userId (in real scenario, backend would handle this)
    // For demo, return all or subset based on userId hash
    const userHash = userId.split('').reduce((acc, ch) => acc + ch.charCodeAt(0), 0);
    const filteredTransactions = mockTransactions.slice(0, Math.max(3, (userHash % 6) + 1));
